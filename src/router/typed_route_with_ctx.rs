...
pub struct TypedRouteWithCtx<T: FromBody, R, Ctx = ()> {
    pub method: Method,
    pub path: PathBuf,
    pub handler: RouteWithCtxHandler<T, R, Ctx>,
}

impl<T, R, Ctx> Route<Ctx> for TypedRouteWithCtx<T, R, Ctx>
where
    ...
    Ctx: Debug + Send + Sync,
{
    ...
    fn handle<'a>(
        ...
        Box::pin(async move {
            ...
            self.handler.handle(request, ctx).await.send(send).await
        })
    }
}
